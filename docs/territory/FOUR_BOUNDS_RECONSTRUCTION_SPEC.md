...
### §12.2 最终实现规格：对偶图标签匹配 min-cut（2026-08-29，文献确认）

任务重述：**客户围栏=确定的 n 个基础单元；任务=用道路/河流/行政边界
找到相同的基础单元集合。** 数学形式 = 基础单元划分的对偶图上，
找包围种子单元的最小代价闭合割线（dual cycle），割线各段与四至
语义的弧标签匹配。

文献支撑（2026-08-29 检索）：
- 平面图 cut-cycle duality：min s-t cut ⟺ 对偶图最小代价圈
  （arXiv 1011.2843； Erickson 几何学院讲义； 近线性算法已有）
- graph cut 分割与收缩偏置解法：Vicente et al. CVPR08（连通性先验）
- NL→空间谓词→图约束查询：Spatial-RAG/GeoRAG 一脉（2025-2026 综述）

**容量函数（替代 §12.1 的距离衰减——标签匹配才是四至的真实语义）**：
每条单元邻接弧携带其所属线网的名称（路名/河名/界名）。割该弧的容量：
  cap(e) = len(e) ×
    { 0.05                        弧名与该侧四至条款名精确匹配
    ; 1 + (名称编辑距离/500m)²    部分匹配（华南快速 vs 华南大桥）
    ; 10                          无名称或完全不匹配 }
方位先验：弧中点相对中心的方向与条款方位不符 → ×5。

管线：四至解析（方位+要素名）→ 弧标签匹配容量 → min s-t cut
（源=中心单元所在面，汇=外部面）→ 源侧单元集 = 围栏。

## §13 范式终极修正：围栏检索，非几何求解（2026-08-29，用户拍板）

**用户定义：四至只是方向性指引，不是强制约束；描述语言完全自由
（街道、地标、区县、路名、面积、口语碎片皆可），目的是用语言认出围栏。**

问题定义修正：
- 旧（废弃）：四至文本 → 硬约束 → 求解边界几何（欠定，六版算法全败）
- 新：任意自然语言描述 → 围栏指纹注册表打分排序 → top-k → 用户确认
  → 单元集合精确回取（100% 一致，无重建误差）

围栏指纹注册表（每围栏自动构建富描述）：
- 行政归属：区、包含/相邻街道（单元库属性聚合）
- 边界路名：语料 v2 方位贴边路名（含河流）
- 中心点位、面积量级、邻接围栏名

检索 = 混合打分：
1. 结构化命中：查询中的街道/区/路名 → 注册表字段
2. 语义相似：查询文本 vs 围栏文本画像（向量兜底自由表达）
3. 数值过滤：中心距离、面积量级
排序 top-k → 用户确认 → 单元集合回取。LLM 负责解析、画像生成、模糊仲裁。

历史定位：§12 单元分配、§12.2 标签匹配 min-cut 降级为"已知全部真值时
的精确表达/验证手段"，不再是主求解路径。

## §14 最终设计自审（3 轮，2026-08-30）

最终设计：基础单元(20,493街区,带街道/区/贴边路名) → 分配台账(单元→经销商)
→ 自由文本经属性索引解析为单元集合做台账变更 → 围栏=名下单元并集
→ 评估=单元重合度(P/R/J)。

自审1 模型层：成立。证据 Zoltners&Sinha1983(Management Science)
"grouping small geographic units into territories" 同构；SKATER(Assunção2006)/
REDCAP(Guo2008,IJGIS) 单元连通聚合。对抗：粒度0.036km²≪最小围栏；业代骑跨=2；
覆盖97%(缺口=水域山体)。保留：单元库随路网更新需重建。

自审2 操作层：成立。证据 Spatial-RAG/GeoRAG(2025-26) 语义解析→结构化谓词→图查询；
索引(423街道/6813路/11区)即谓词库。对抗：POI禁→LLM兜底；方位参考系=街道质心；
同名歧义以(区,街道)键消歧。高频引用纯规则覆盖，长尾LLM。

自审3 评估层：成立。证据 IR P/R 标准；单元一致性规避面积误导。对抗：反推重选=
循环验证仅作基准(真实人驱动)；业代骑跨≈0指标稳定；报告单元数不报面积。

废弃：几何求解六版、四至硬约束、面积指标、中心点分配。


### 坐标系勘误（2026-08-30，用户发现"整体偏移"）
磁盘 region.json/contracts.json **本就存 WGS-84**，但 meta.json 缺省声明
GCJ-02 → pack_from_disk 多做了一次 GCJ→WGS，把全部围栏/中心点/合同坐标
整体推移 ~600m。修复：meta.crs="WGS84"。判决证据：围栏边界→OSM 路
中位距离 72m→6m；服务值=磁盘值（差 0.0m）。
教训：坐标系必须以"贴路距离"实证检验，不能只信声明；业代 CSV（用户画在
高德上）确为 GCJ-02 需转换——**两个数据源坐标系不同，逐个验证**。


### 图层叠加勘误（2026-08-30，用户"这不是偏？"质疑）
实测否定坐标系嫌疑：业代 11 片贴路 0m、穗穗盛 25m（手绘噪声级）、
华新 0m——两套数据都与 WGS 底图对齐。真因 = **两套不同主体的围栏同屏叠加**：
穗穗盛(经销商) 与 业代07/08/09/11 重叠 91%，选中经销商时业代层默认开着，
糊成一片看似偏移。修复：①业代层默认关；②选中经销商自动关业代/区界/街道层
只独显该家围栏（多块分色+面积标注）；③该围栏加粗高亮描边。
教训：**"偏移"投诉先量贴路距离和重叠率再下结论**——本次确实是图层不是坐标。
