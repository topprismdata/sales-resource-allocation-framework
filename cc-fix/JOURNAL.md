...
## 2026-08-31 T-010 实现与验收记录（Phase 5）

### 实现

- 新增 `tools/eval_dual_table.py`，只读取 `data/gz/territory_compiled.json`
  和单元台账数据；不修改 `territory_compile.py`、`allocation_ledger.py`、
  `cc-fix/CONTRACTS.md` 或任何数据包产物。
- 生成 `cc-fix/verify/P5_REPORT.md`，将几何天花板表 A 与跨模块反解表 B
  分列，并在报告顶部原样保留强制免责声明。
- 表 B 对 35 条有 S 片的记录逐词调用 `Ledger.resolve_units`，保留所有样本，
  包含 Jaccard=0 的记录；每条记录带 `[自动生成语料]` 标签。

### 实测结果

- 70 条编译记录中 35 条可编译、35 条失效；失效归属地为珠海13、中山7、
  韶关3、始兴1、其他11。
- 表 A IoU 中位/最小/最大为 0.9960/0.7550/1.0000；cover 中位/最小/最大
  为 99.84%/75.53%/100.00%；`engine_J` 35/35 为 1.0，报告中标为
  `J_selfcover` 自证指标，不作为还原能力。
- 表 B Jaccard 中位/均值/最小/最大为 0.3973/0.3983/0.0000/0.9288，
  解析失败词 36 个：界带表达32词/18围栏，“一带”模糊3词/3围栏，
  `沿珠江` 索引为空1词/1围栏。除界带外没有影响超过30%样本的第二类模式，
  未触发上报条件。
...
完整断言输出见 `cc-fix/verify/T-010-verify.txt`。当前未执行 Git commit。

## 2026-08-31 T-010 验收通过（Phase 5 完成：评测口径双表）

**架构层独立执行断言**：J1–J7 全 PASS（免责齐全/两表分列/自证已标注/
覆盖率已披露/语料标签齐备/无合并评分/幂等），J8 = 23 passed。
`tools/eval_dual_table.py` 无硬编码路径。

### ★★ 核心量化结果

| 指标 | 值 | 性质 |
|---|---:|---|
| `J_selfcover` | **1.0000（35/35，无一例外）** | L0 自证，禁止对外引用 |
| `IoU` | 中位 0.9960，**最小 0.7550** | 表 A 几何天花板（真实） |
| `cover` | 中位 99.84% | 表 A |
| **`J_recall`** | **中位 0.3973**，均值 0.3983，最小 0.0000，最大 0.9288 | **L1 跨模块反解（真实）** |
| 覆盖率 | **35/70** | 另 35 条因单元库仅覆盖广州而完全无法编译 |

**自证与真实链路的差距达 0.6 个 Jaccard**（1.0000 → 0.3973）。

### 解析失败模式归类（36 个失败词，可行动）

| 模式 | 失败词数 | 影响围栏数 |
|---|---:|---:|
| **界带表达**（`X至A—B界` / `X至行政区界`） | **32** | **18 / 35** |
| 「一带」表述触发模糊匹配 | 3 | 3 |
| 沿路/地物词无法命中索引 | 1 | 1 |

**结论**：`J_recall` 偏低的主因高度集中——`territory_compile.verbalize()`
会生成界带表达（如「东至昌岗街道—新港街道界」），而
`allocation_ledger.resolve_units()` **完全不支持该语法**，
一条失败即可使整条围栏 Jaccard 归零（如大源鸿欣：唯一人话句为
「西至白云区界」→ 解出 0 → J=0.0000）。
